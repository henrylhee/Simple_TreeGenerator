...
namespace Gen
{
    public class Internode
    {
        ...
        public Internode(Vector3 position, Vector3 initGrowDir, float thickness)
        {
            ...
            direction = Helper.getRandomVectorInCone(GraphModel.Instance.RandomGrowthConeAngle, initGrowDir);
            this.thickness = thickness > GraphModel.Instance.MinThicknessAbsolute ? thickness : GraphModel.Instance.MinThicknessAbsolute;
            //this.thickness = thickness;
        }

        public Internode FindNext(float length, float thickness)
        {
            return new Internode(position + (direction * length), Helper.getRandomVectorInCone(GraphModel.Instance.RandomGrowthConeAngle, direction), thickness);
        }
        ...
        public Vector3 GetSplitGrowDir(float phyllo)
        {
            ...
            return Quaternion.AngleAxis(GraphModel.Instance.BranchingAngle, Vector3.Cross(direction, resultPhyllo)) * direction;
        }
    }
}
